...
void GameStateLevel1Init()
{
	...
	planetSource = new SpriteSource(1, 1, planetTexture);
	idleSource = new SpriteSource(1, 1, pTextureMonkeyIdle);
	walkSource = new SpriteSource(3, 3, pTextureMonkeyWalk);
	jumpSource = new SpriteSource(1, 1, pTextureMonkeyJump);
	fontSource = new SpriteSource(16, 6, pTextureFontSheet);

	if (planet)
	{
		SpritePtr holder = planet->GetSprite();

		holder->SetMesh(unitMesh);
		holder->SetSpriteSource(planetSource);
	}
	...
	if (text)
	{
		SpritePtr textSprite = text->GetSprite();

		textSprite->SetMesh(textMesh);
		textSprite->SetSpriteSource(fontSource);

		sprintf_s(livesBuffer, 16, "lives: %d", numLives1);

		textSprite->SetText(livesBuffer);
	}
...
void GameStateLevel1SetMonkeyState(GameObjectPtr gameObject, MonkeyStates newState)
{
	if (monkeyState != newState)
	{
		...
		AnimationPtr animation = gameObject->GetAnimation();
		...
		{
		case MonkeyIdle:
			sprite->SetMesh(unitMesh);
			sprite->SetSpriteSource(idleSource);
			animation->Play(1, 0.0f, false);
			break;

		case MonkeyWalk:
			sprite->SetMesh(threeMesh);
			sprite->SetSpriteSource(walkSource);
			animation->Play(8, 0.05f, true);
			break;

		case MonkeyJump:
			sprite->SetMesh(unitMesh);
			sprite->SetSpriteSource(jumpSource);
			animation->Play(1, 0.0f, false);
			break;
		}
	}
...
bool GameStateLevel1IsColliding(GameObjectPtr objectA, GameObjectPtr objectB)
{
	if (objectA && objectB)
	{
		Vector2D positionA = *objectA->GetTransform()->GetTranslation();
		Vector2D positionB = *objectB->GetTransform()->GetTranslation();
