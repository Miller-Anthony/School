...
GameObjectPtr GameObjectClone(const GameObjectPtr other)
{
	if (other)
	{
		...
		if (holder)
		{
			...
			if (other->collider)
			{
				GameObjectAddCollider(holder, ColliderClone(other->collider));
			}
...
void GameObjectRead(GameObjectPtr gameObject, Stream stream)
{
	if (gameObject && stream)
	{
		...
		while (true)
		{
			...
			{
				BehaviorPtr asteroid = BehaviorAsteroidCreate();
				BehaviorRead(asteroid, stream);
				GameObjectAddBehavior(gameObject, asteroid);
			}
			else if (strcmp(holder, "BehaviorHudText") == 0)
			{
				BehaviorPtr hudText = BehaviorHudTextCreate();
				BehaviorRead(hudText, stream);
				GameObjectAddBehavior(gameObject, hudText);
			}
			else if (strcmp(holder, "Collider") == 0)
			{
